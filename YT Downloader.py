...
def build_video_command(url, video_path, ffmpeg_path):
    command = [
        "yt-dlp",
        "-f",
    ...
    if ffmpeg_path:
        command.insert(5, "--ffmpeg-location")
        command.insert(6, ffmpeg_path)
    return command


def build_audio_command(url, audio_path, ffmpeg_path):
    command = [
        "yt-dlp",
        "-f",
        "bestaudio/best",
    ...
    if ffmpeg_path:
        command.insert(7, "--ffmpeg-location")
        command.insert(8, ffmpeg_path)
    return command
...
class DownloaderGUI:
    ...
    def _start_download(self, mode):
        ...
        if mode == "video":
            command = build_video_command(url, video_path, ffmpeg_path)
            action = "Downloading video"
        else:
            command = build_audio_command(url, audio_path, ffmpeg_path)
